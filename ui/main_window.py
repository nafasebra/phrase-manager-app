...
class MainWindow(ctk.CTk):
    ...
    def _make_card(self, phrase):
        ...
        if phrase.tags:
            tags_text = "  ".join(
                f"#{t.strip()}" for t in phrase.tags.split(",") if t.strip()
            )
            ctk.CTkLabel(
                card,
                text=tags_text,
    ...
    def _copy(self, phrase):
        ...
        self.status_label.configure(text=f"'{phrase.title}' copied!")
    ...
    def _delete(self, phrase):
        dialog = ctk.CTkInputDialog(
            text=f"Type DELETE to remove '{phrase.title}'",
